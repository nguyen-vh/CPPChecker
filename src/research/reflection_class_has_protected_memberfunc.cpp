...
concept class_has_protected_memberfunc = []() {
  ...
  for (auto mem : members) {
    if (!std::meta::is_protected(mem)) continue;
...
auto main(int /*argc*/, char* /*argv*/[]) -> int {
  std::cout << class_has_protected_memberfunc<"XY"_ls, "foos"_ls> << std::endl;
  std::cout
      << class_has_protected_memberfunc<"X"_ls, "foos"_ls, double> << std::endl;
  std::cout << class_has_protected_memberfunc<"X"_ls, "foo"_ls> << std::endl;
  std::cout
      << class_has_protected_memberfunc<"X"_ls, "foo"_ls, double> << std::endl;
  std::cout << class_has_protected_memberfunc<"X"_ls, "foo"_ls, double,
                                              double> << std::endl;
  std::cout << class_has_protected_memberfunc<"XY"_ls, "foo"_ls, double,
                                              std::string> << std::endl;

  std::cout << class_has_protected_memberfunc<"X"_ls, "foo"_ls, double,
                                              std::string> << std::endl;
  std::cout << class_has_protected_memberfunc<"X"_ls, "foo"_ls, double, int,
                                              std::string> << std::endl;
  return 0;
}
