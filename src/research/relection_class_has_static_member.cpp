...
concept class_has_static_member_variable = []() {
  ...
  for (auto mem : members) {
    ...
    if (!std::meta::is_variable(mem)) continue;
    ...
    if (std::meta::has_identifier(mem)) {
      ...
      if (std::meta::type_of(mem) == ^^TYPE) return true;
    }
  }
  return false;
}();
...
auto main(int /*argc*/, char* /*argv*/[]) -> int {
  std::cout << class_has_static_member_variable<"X"_ls, "fooos"_ls,
                                                double> << std::endl;

  std::cout
      << class_has_static_member_variable<"X"_ls, "fooos"_ls> << std::endl;

  std::cout << class_has_static_member_variable<"XS"_ls, "fooos"_ls,
                                                double> << std::endl;
  return 0;
}