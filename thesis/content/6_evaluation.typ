...
The REST API server and web UI stack functioned without error or unexpected behavior, and the code sent for evaluation did not compromise the integrity of the system. The mean RTT for all three tasks is 8 seconds. The fastest recorded RTT was 5 seconds, while the longest recorded RTT was 12 seconds. The testing was conducted over a university Virtual Private Network (VPN) to access the university server from outside.
