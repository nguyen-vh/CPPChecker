...
A thorough examination of the namespaces reveals the presence of four distinct ones. Notably, the "check" namespace in @fig:macro_class_has_memberfunc, commencing on line 159, is employed to avoid the pollution of the global namespace. The namespaces "condition" and "tester" within the "condition" namespace on line 148 establish the foundation for the class lookup to proceed without failure in the event that it is not provided by the checked code. As illustrated in @fig:macro_fallback, the definition for the fallback class is located on line 77.
The final namespace is concealed and is designated by a macro named "NAMESPACE_TO_CHECK" because it is subject to alteration based on the specific use case. During the process of compilation, it can be modified to a different namespace. By examining the @fig:macro_class_has_memberfunc line 151, it becomes evident that the template employs the C++ _Concepts_ to ascertain the presence of a member function, instead of using the older SFINAE approach with multiple templates and _std::void_t_. The function requires a _typename T_ argument, which is supplied with the desired class for searching. The problem is that if the provided class does not exist, this will lead to a compile error. Consequently, a function within the specified namespace, designated as "NAMESPACE_TO_CHECK::condition", is invoked. The "NAMESPACE_TO_CHECK::condition" namespace utilizes the "condition::tester" namespace internally and invokes the template. Accordingly, the template call with the type T will first utilize the class contained within "NAMESPACE_TO_CHECK". In the event that the class is unavailable, the struct "FallbackClass" will be employed as it is also "globally" available within that same namespace.
...
The utilization of the aforementioned code can be achieved through the implementation of a specific configuration. This configuration involves the placement of the code inside a namespace, in @fig:macro_usecase "check_in_here". Additionally, the execution of the macro requires the invocation of the class name and the function name. This will result in the generation of a function within the "check" namespace, with the provided names, in this case "check::class_X_has_memberfunc_foo_v". This function will return a boolean value, shown in @fig:macro_usecase.
...
A potential drawback of the code is that it will generate an error if multiple macros with identical inputs are utilized. Furthermore, it should be noted that the macros, which are designed to verify the return and input types, are incapable of testing a C++ type that utilizes multiple words, such as "long long int". This is due to a conflict with the name generation for the function. One possible solution to this issue is to define aliases in order to circumvent the conflict.
